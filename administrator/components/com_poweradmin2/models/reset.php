...
class JSNPowerAdmin2ModelReset extends JModelLegacy
{
	...
	public function importSampleData($sample, $keep_reset, $keep_user, $admin_email = '', $admin_username = '', $admin_password = '')
	{
		try
		{
			...
			if ($keep_reset == 'yes')
			{
				// Discover extensions left after resetting.
				...
				if (class_exists('InstallerModelDiscover'))
				{
					...
					if (count($exts = $model->getItems()))
					{
						// Reinstall extensions left after resetting.
						foreach ($exts as $ext)
						{
							$input->set('cid', $ext->extension_id);
							$input->set('tool_redirect', 0);
							$model->discover_install();
						}
					}
				}

				// Restore extension parameters.
				foreach ($extensions as $extension)
				{
					$qry = $dbo->getQuery(true);

					$qry->update('#__extensions')
						->set('params = ' . $dbo->quote($extension['params']))
						->where('type = ' . $dbo->quote($extension['type']))
						->where('folder = ' . $dbo->quote($extension['folder']))
						->where('element = ' . $dbo->quote($extension['element']));
